...
public class ProbCog extends Defense{
	...
	private void FirstLayerClassification(int bid) {
		...
		for (int k = 0,aid=k; k < ecommerce.getBuyerList().size(); k++) {
			// int aid = k;
			if(bid == aid)continue;
		...
		m_popQuality = firstlayerDishonestyPred / ecommerce.getBuyerList().size();
	}
	...
	private void SecondLayeradvisersClassification() {
		...
		// df.format(epsilon) );
		for (int k = 0; k <ecommerce.getBuyerList().size(); k++) {
			int aid = k;
			double A_diff = 0.0;
